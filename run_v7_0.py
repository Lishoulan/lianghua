...
def train_models(all_stock_data, all_featured_data, oamv_state_dict,
    ...
    try:
        ...
        if kan_preds.sum() > 0:
            ...
            print(f"  KAN Precision@0.65: {kan_precision:.2%}")
    except Exception as e:
        print(f"  KAN training failed: {e}, using CatBoost only")
        kan_trainer = None
...
def run_backtest(stock_limit=300):
    print("=" * 120)
    print("v7.0: SSA降噪 + ADDM漂移检测 + RADE集成(CatBoost+KAN) + Bootstrapped MVO")
    print("=" * 120)
    print(f"\nConfig:")
    print(f"  SSA window: 10, signal groups: 2")
    print(f"  ADDM: AR(3), PH threshold=5.0, delta=0.005")
    ...
    print(f"  KAN: hidden_dim=16, grid_size=5, spline_order=3")
    ...
    drift_detector = ADDMDriftDetector(ar_order=3, ph_threshold=2.0, ph_delta=0.01)
    ...
    print("\n[Stage 6] 季度Walk-Forward滚动验证 (v7.0)")
    print("-" * 80)
    ...
    print("\n" + "=" * 120)
    print("v7.0 WALK-FORWARD RESULTS")
    print("=" * 120)
    ...
    data = {
        'strategy': 'v7.0_SSA_ADDM_RADE_MVO',
        'config': {
            'ssa_window': 10,
            'ssa_signal_groups': 2,
            'addm_ar_order': 3,
            'addm_ph_threshold': 5.0,
            'rade_gamma': 0.5,
            'mvo_scenarios': 500,
            'mvo_block_size': 5,
    ...
    report_file = output_dir / f"v7.0_{timestamp}.json"
