...
class SimPosition:
    def __init__(self, ts_code, name, entry_date, entry_price, entry_prob, entry_atr):
...
def run_paper_trade_sim():
    print("=" * 100)
    print(f"v7.0 模拟交易回放 - 最近 {SIM_DAYS} 个交易日逐日明细")
    print(f"SSA降噪 + ADDM漂移检测 + RADE集成(CatBoost+KAN) + Bootstrapped MVO")
        ...
        if day['sells']:
            ...
            for s in day['sells']:
                print(f"     {s['code']} {s['name']}  卖出价={s['price']:.2f}  买入价={s['entry_price']:.2f}  "
                      f"收益={s['profit_pct']:+.2f}%  持仓={s['hold_days']}天  原因={s['reason']}")

        if day['buys']:
            print(f"  🟢 买入:")
            for b in day['buys']:
                mvo_str = f"  MVO权重={b['mvo_weight']:.2f}" if b.get('mvo_weight') is not None else ""
                print(f"     {b['code']} {b['name']}  买入价={b['price']:.2f}  RADE概率={b['prob']:.1%}  ATR={b['atr']:.2f}{mvo_str}")
        ...
        if day['positions']:
            ...
            for code, p in day['positions'].items():
                ...
                print(f"     {code} {p['name']}  现价={p['current_price']:.2f}  "
                      f"收益={p['profit_pct']:+.2f}%  持仓={p['hold_days']}天  峰值回撤={dd:.1f}%")
    ...
    result_file = output_dir / f"paper_trade_{timestamp}.json"
