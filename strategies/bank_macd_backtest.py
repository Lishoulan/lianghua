...
def get_bank_stocks():
    ...
    bank_stocks = all_stocks[
        (all_stocks['industry'] == '银行') | 
        (all_stocks['name'].str.contains('银行', na=False))
    ]
    
    print(f"✅ 共获取 {len(bank_stocks)} 只银行股")
...
def calculate_macd(df, fast=12, slow=26, signal=9):
    ...
    # MACD柱上穿0轴：前一根<0，当前>=0
    df['MACD_cross_up'] = (df['MACD'] >= 0) & (df['MACD'].shift(1) < 0)
    
    # MACD柱下穿0轴：前一根>=0，当前<0
    df['MACD_cross_down'] = (df['MACD'] < 0) & (df['MACD'].shift(1) >= 0)
    
    return df


def get_stock_data(ts_code, start_date='20210101', end_date='20260516'):
    """获取股票数据"""
    try:
...
def run_bank_stocks_backtest():
    ...
    for _, row in bank_stocks.iterrows():
        ...
        df = get_stock_data(ts_code, '20210101', '20260516')
    ...
    print("\n" + "=" * 100)
    print("📊 银行股MACD策略回测结果汇总（按收益率排序）")
    print("=" * 100)
    print(f"{'排名':>4} {'股票':<12} {'代码':<12} {'期初价格':>10} {'期末价格':>10} {'MACD策略':>12} {'交易次数':>8} {'胜率':>8}")
    print("-" * 100)
    
    for i, r in enumerate(results, 1):
        print(f"{i:>4}. {r['name']:<12} {r['code']:<12} "
    ...
    print("📈 总体统计")
    print("=" * 100)
    print(f"银行股总数: {total_count}")
    print(f"正收益股票: {win_count} ({win_count/total_count*100:.1f}%)")
    print(f"负收益股票: {total_count - win_count} ({(total_count-win_count)/total_count*100:.1f}%)")
...
def save_results(results, avg_return, win_count, total_count):
    ...
    report_file = output_dir / f"bank_macd_{timestamp}.json"
