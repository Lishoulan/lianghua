...
_pip_libs = str(Path(__file__).parent / "pip_libs")
...
from ml_strategy.alpha_glass import AlphaGlassTrainer
from ml_strategy.proteus_stress import ProteuSGenerator
...
def run_paper_trade_sim():
    print("=" * 100)
    print("v9.0 几何与可解释集成 - Path Signatures + AlphaGlass + ProteuS")
    print(f"最近 {SIM_DAYS} 个交易日逐日明细")
    print("SSA降噪 + ADDM漂移检测 + RADE集成(CatBoost+ChebyKAN) + Path Signatures + AlphaGlass + CostAware MVO + Sqrt Impact")
    print("无菌清洗 + 分歧特征 + AMSE损失 + 漂移重训 + 几何路径签名 + 可解释组合")
    ...
    print(f"  PathSignatureBuilder: truncation=2, path_dims=3, path_length=5, lead_lag=False")
    ...
    print("\n[3/8] 加载股票数据 (with Sterile Cleaning + Disagreement + SSA denoising + Path Signatures)...")
    ...
    print("\n[6/8] 训练 RADE集成模型 (CatBoost + ChebyKAN, AMSE Loss) + AlphaGlass...")
    ...
    alpha_glass_trainer = None
    try:
        top_stock_codes = list(all_stock_data.keys())[:5]
        stock_returns_dict = {}
        for code in top_stock_codes:
            sdf = all_stock_data[code]['data']
            if 'Close' in sdf.columns and len(sdf) > 20:
                rets = sdf['Close'].pct_change().dropna()
                if len(rets) > 20:
                    stock_returns_dict[code] = rets

        if len(stock_returns_dict) >= 2:
            returns_df = pd.DataFrame(stock_returns_dict)
            returns_df = returns_df.dropna()
            if len(returns_df) > 30:
                alpha_glass_trainer = AlphaGlassTrainer(
                ...
                alpha_glass_trainer.train(returns_df)
                print(f"  AlphaGlass训练完成: {len(returns_df.columns)} 资产, {len(returns_df)} 样本")
            else:
                print(f"  AlphaGlass: 收益数据不足 ({len(returns_df)} 行), 跳过")
        else:
            print(f"  AlphaGlass: 有效资产不足 ({len(stock_returns_dict)}), 跳过")
    except Exception as e:
        print(f"  AlphaGlass训练失败: {e}, 将使用MVO回退")
        alpha_glass_trainer = None

    print(f"\n[7/8] 模拟交易回放 (最近 {SIM_DAYS} 个交易日)...")
    print("=" * 100)
    ...
    for day_idx, current_date in enumerate(sim_dates):
        ...
        if oamv_weekly and not is_panic and len(positions) < MAX_PORTFOLIO_STOCKS:
            ...
            if top_n >= 2:
                ...
                alpha_glass_weights = None
                if alpha_glass_trainer is not None:
                    try:
                        candidate_returns = {}
                        for code in candidate_codes:
                            if code in all_stock_data:
                                ...
                                if 'Close' in sdf.columns:
                                    rets = sdf['Close'].pct_change().dropna()
                                    if len(rets) > 20:
                                        candidate_returns[code] = rets
                        if len(candidate_returns) >= 2:
                            cand_ret_df = pd.DataFrame(candidate_returns)
                            cand_ret_df = cand_ret_df.dropna()
                            if len(cand_ret_df) > 10:
                                alpha_glass_weights = alpha_glass_trainer.predict_weights(cand_ret_df)
                                if alpha_glass_weights is not None and len(alpha_glass_weights) > 0:
                                    valid_codes_ag = list(candidate_returns.keys())
                                    print(f"  AlphaGlass权重预测: {dict(zip(valid_codes_ag, [f'{w:.3f}' for w in alpha_glass_weights]))}")
                    except Exception:
                        alpha_glass_weights = None

                if alpha_glass_weights is not None:
                    try:
                        valid_codes_ag = list(candidate_returns.keys())
                        weighted_candidates = []
                        for code, w in zip(valid_codes_ag, alpha_glass_weights):
                            for c in buy_candidates:
                                if c['ts_code'] == code:
                                    c['mvo_weight'] = w
                                    c['weight_source'] = 'AlphaGlass'
                                    weighted_candidates.append(c)
                                    break
                        for c in buy_candidates[:top_n]:
                            if c['ts_code'] not in [wc['ts_code'] for wc in weighted_candidates]:
                                c['mvo_weight'] = 0.0
                                c['weight_source'] = 'AlphaGlass(unmatched)'
                                weighted_candidates.append(c)
            ...
            for candidate in buy_candidates:
                ...
                day_buys.append({
                    ...
                    'weight_source': candidate.get('weight_source', 'MVO'),
                    'impact_slippage': candidate['impact_slippage'],
                })

        if day_idx > 0:
            market_vol = 0.02
            if 'ATR14' in index_df.columns and current_date in index_df.index:
                atr_val = float(index_df.loc[current_date, 'ATR14'])
                close_val = float(index_df.loc[current_date, 'Close'])
                if close_val > 0 and not pd.isna(atr_val):
                    market_vol = atr_val / close_val
            drift_detector.set_market_volatility(market_vol)

        if drift_detector.should_retrain(day_idx):
            print(f"  ⚠️ ADDM漂移触发重训! Day {day_idx}")
            drift_detector.mark_retrained(day_idx)
    ...
    for day in daily_log:
        ...
        if day['buys']:
            print(f"  🟢 买入:")
            for b in day['buys']:
                mvo_str = f"  {b.get('weight_source', 'MVO')}权重={b['mvo_weight']:.2f}" if b.get('mvo_weight') is not None else ""
    ...
    print(f"\n[8/8] ProteuS 压力测试 (ADDM漂移检测器)...")
    print("=" * 100)
    try:
        proteus = ProteuSGenerator(n_scenarios=500, seed=42)
        addm_configs = [
            {'ar_order': 2, 'ph_threshold': 1.5, 'ph_delta': 0.005, 'use_vol_filter': True, 'decay_lambda': 0.003},
            {'ar_order': 3, 'ph_threshold': 2.0, 'ph_delta': 0.01, 'use_vol_filter': True, 'decay_lambda': 0.005},
            {'ar_order': 3, 'ph_threshold': 2.5, 'ph_delta': 0.02, 'use_vol_filter': True, 'decay_lambda': 0.008},
            {'ar_order': 4, 'ph_threshold': 3.0, 'ph_delta': 0.03, 'use_vol_filter': False, 'decay_lambda': 0.01},
        ]
        stress_results = proteus.stress_test_addm(
            drift_detector=drift_detector,
            configs=addm_configs,
            sim_days=SIM_DAYS,
        )
        print(f"  ProteuS场景数: {proteus.n_scenarios}")
        print(f"  ADDM配置数: {len(addm_configs)}")
        if stress_results is not None:
            if isinstance(stress_results, dict):
                for config_name, result in stress_results.items():
                    if isinstance(result, dict):
                        retrain_count = result.get('retrain_count', 'N/A')
                        false_positive_rate = result.get('false_positive_rate', 'N/A')
                        detection_delay = result.get('detection_delay', 'N/A')
                        print(f"  配置 [{config_name}]: 重训次数={retrain_count}, 误报率={false_positive_rate}, 检测延迟={detection_delay}")
                    else:
                        print(f"  配置 [{config_name}]: {result}")
                ...
                for i, result in enumerate(stress_results):
                    if isinstance(result, dict):
                        ...
                        false_positive_rate = result.get('false_positive_rate', 'N/A')
                        detection_delay = result.get('detection_delay', 'N/A')
                        print(f"  配置 [{i}]: 重训次数={retrain_count}, 误报率={false_positive_rate}, 检测延迟={detection_delay}")
                    else:
                        print(f"  配置 [{i}]: {result}")
            else:
        ...
        print(f"  ProteuS压力测试失败: {e}")

    output_dir = Path(__file__).parent / "paper_trade_results"
    output_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    result_file = output_dir / f"paper_trade_v90_{timestamp}.json"
    with open(result_file, 'w', encoding='utf-8') as f:
        json.dump({
            ...
            'stress_test': stress_results if 'stress_results' in dir() else None,
