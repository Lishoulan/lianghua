...
def train_catboost(all_stock_data, all_featured_data, oamv_state_dict,
    ...
    if val_preds.sum() > 0:
        ...
        print(f"  Samples: {len(all_samples)}, Precision@0.65: {precision:.2%}")
    return model


def run_portfolio_backtest(all_stock_data, all_featured_data, model, oamv_filter,
                           panic_breaker, test_start, test_end):
        ...
        if weekly_ok and backtester.can_buy():
            ...
            for ts_code, info in all_stock_data.items():
                ...
                if prob >= CATBOOST_BUY_THRESHOLD:
                    ...
                    buy_candidates.append({
                        ...
                        'amplitude_20': amp if not pd.isna(amp) else 0,
                    })
            ...
            for candidate in buy_candidates:
                ...
                if pd.isna(price) or price <= 0:
                    continue
                amp = candidate['amplitude_20']
                if amp > 0 and amp < FRICTION_COST_PCT * MIN_AMPLITUDE_MULT:
                    continue
...
def run_backtest(stock_limit=300):
    print("=" * 120)
    print("v6.2: Universe AMV + EMA-5周线 + 季度滚动训练 + 动态成本阀门 + 熔断器")
    print("=" * 120)
    print(f"\nConfig:")
    print(f"  Train window: {TRAIN_WINDOW_MONTHS} months rolling")
    print(f"  WF quarters: {len(WF_QUARTERS)}")
    print(f"  Weekly filter: EMA-5 (adaptive)")
    print(f"  Friction cost gate: amplitude >= {FRICTION_COST_PCT * MIN_AMPLITUDE_MULT:.2f}%")
    print(f"  OAMV source: Universe AMV (stock pool)")
    print(f"  Panic breaker: breadth >= 85% below 20-day MA OR limit-down >= 150")
    ...
    print("\n[Stage 3.5] 市场宽度熔断器 (Panic Circuit Breaker)")
    print("-" * 80)
    ...
    for wf_idx, wf_window in enumerate(WF_QUARTERS, 1):
        ...
        backtester = run_portfolio_backtest(
            ...
            panic_breaker, wf_window['test_start'], wf_window['test_end']
        )
    ...
    print("\n" + "=" * 120)
    print("v6.2 WALK-FORWARD RESULTS")
    print("=" * 120)
    ...
    data = {
        'strategy': 'v6.2_panic_breaker_l2reg',
        'config': {
            'oamv_source': oamv_filter.data_source,
            'weekly_filter': 'EMA-5 adaptive',
            'train_window_months': TRAIN_WINDOW_MONTHS,
            'wf_quarters': len(all_wf_results),
    ...
    report_file = output_dir / f"v6.2_{timestamp}.json"
