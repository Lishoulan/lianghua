...
WF_WINDOWS = [
    {'train_start': '20210101', 'train_end': '20231231', 'test_start': '20240101', 'test_end': '20241231'},
    {'train_start': '20220101', 'train_end': '20241231', 'test_start': '20250101', 'test_end': '20260520'},
]
...
def get_all_a_stocks(limit=None, filter_st=True):
    ...
    try:
        ...
        if filter_st:
            ...
            print(f"Filtered ST: {before} -> {len(a_stocks)} (removed {before - len(a_stocks)})")
        if limit:
...
def get_daily_basic(ts_code='000001.SH', start_date='20210101', end_date='20260520'):
    print(f"Fetching daily_basic for {ts_code} (live chips data)...")
    try:
        all_data = []
        ...
        while current_start < end_date:
            df = pro.daily_basic(
                ...
                end_date=end_date,
                fields='trade_date,close,turnover_rate,turnover_rate_f,volume,amount,total_mv,circ_mv'
            )
            if df is None or df.empty:
                break
            all_data.append(df)
        ...
        print(f"  daily_basic: {len(result)} rows, {result.index[0]} to {result.index[-1]}")
        return result
    except Exception as e:
...
def load_industry_data(start_date='20210101', end_date='20260520'):
    print("Loading industry data (sw_daily first, ETF fallback)...")
    industry_j_cache = {}
    sw_count = 0
    etf_count = 0
...
def load_all_stock_data(stock_list, index_df, industry_j_cache, start_date='20210101', end_date='20260520'):
    print("Loading and preprocessing all stock data...")
    all_stock_data = {}
    all_featured_data = {}
...
def train_catboost(all_stock_data, all_featured_data, oamv_state_dict,
                   train_start, train_end):
    print(f"Training CatBoost: {train_start} ~ {train_end}")
    all_samples = []
    discretizer = FeatureDiscretizer()
    ...
    for idx, ts_code in enumerate(ts_codes, 1):
        if idx % 50 == 0:
            print(f"  Collecting: {idx}/{len(ts_codes)}, samples: {len(all_samples)}")
    ...
    if val_preds.sum() > 0:
        ...
        print(f"  Val precision@{CATBOOST_BUY_THRESHOLD}: {precision:.2%}")

    return model


def run_portfolio_backtest(all_stock_data, all_featured_data, model, oamv_filter,
                           test_start, test_end):
    print(f"Portfolio backtest: {test_start} ~ {test_end}")
    ...
    if len(eval_dates) == 0:
        print("  No trading dates in test period!")
        return backtester
    ...
    for date_idx, current_date in enumerate(eval_dates, 1):
        if date_idx % 20 == 0:
            ...
            print(f"  {date_idx}/{len(eval_dates)}, Portfolio: {pv:,.0f}, Positions: {len(backtester.positions)}")

        daily_ok = oamv_filter.is_trading_allowed(current_date, require_weekly=False)
        weekly_ok = oamv_filter.is_trading_allowed(current_date, require_weekly=True)
        oamv_state = 1 if daily_ok else 0
        ...
        if weekly_ok and backtester.can_buy():
            ...
            top_n = buy_candidates[:backtester.get_available_slot_count() * 2]
...
def run_backtest(stock_limit=300):
    print("=" * 120)
    print("v6.0: 活筹0AMV + 周线过滤 + sw_daily行业 + Walk-Forward验证")
    print("=" * 120)

    print("\n[Stage 1] 0AMV迟滞状态机 (活筹指数 + 周线过滤)")
    print("-" * 80)
    ...
    daily_basic_df = get_daily_basic('000001.SH', '20210101', '20260520')
    ...
    oamv_filter.fit(index_df, daily_basic_df=daily_basic_df)
    ...
    print("\n[Stage 1.5] 行业数据 (sw_daily优先 + ETF备选)")
    print("-" * 80)
    ...
    all_stock_data, all_featured_data = load_all_stock_data(
        stock_list, index_df, industry_j_cache, '20210101', '20260520'
    )
    if not all_stock_data:
        print("No stock data loaded!")
        return

    print("\n[Stage 3] Walk-Forward 滚动验证")
    print("-" * 80)

    all_wf_results = []

    for wf_idx, wf_window in enumerate(WF_WINDOWS, 1):
        print(f"\n--- WF Window {wf_idx}/{len(WF_WINDOWS)} ---")
        print(f"  Train: {wf_window['train_start']} ~ {wf_window['train_end']}")
        print(f"  Test:  {wf_window['test_start']} ~ {wf_window['test_end']}")
        ...
        if model is None:
            print("  Model training failed, skipping window")
            continue

        backtester = run_portfolio_backtest(
            all_stock_data, all_featured_data, model, oamv_filter,
            wf_window['test_start'], wf_window['test_end']
        )

        summary = backtester.get_summary()
        print(f"\n  WF Window {wf_idx} Results:")
        ...
        print(f"    Trades: {summary.get('trade_count', 0)}, Win rate: {summary.get('win_rate', 0):.1f}%")
        print(f"    Avg hold: {summary.get('avg_hold_days', 0):.1f} days")
    ...
    print("\n" + "=" * 120)
    print("WALK-FORWARD VALIDATION RESULTS (v6.0)")
    print("=" * 120)

    for wf_idx, result in enumerate(all_wf_results, 1):
        s = result['summary']
        w = result['window']
        print(f"\n  Window {wf_idx}: Test {w['test_start']} ~ {w['test_end']}")
        print(f"    Return: {s.get('total_return', 0):+.2f}%, Sharpe: {s.get('sharpe', 0):.3f}, "
              ...
              f"Trades: {s.get('trade_count', 0)}, AvgHold: {s.get('avg_hold_days', 0):.1f}d")

    if all_wf_results:
        avg_return = np.mean([r['summary'].get('total_return', 0) for r in all_wf_results])
        avg_sharpe = np.mean([r['summary'].get('sharpe', 0) for r in all_wf_results])
        avg_maxdd = np.mean([r['summary'].get('max_drawdown', 0) for r in all_wf_results])
        avg_winrate = np.mean([r['summary'].get('win_rate', 0) for r in all_wf_results])
        total_trades = sum(r['trade_count'] for r in all_wf_results)

        print(f"\n  Walk-Forward Aggregate:")
        print(f"    Avg return: {avg_return:+.2f}%")
        print(f"    Avg Sharpe: {avg_sharpe:.3f}")
        print(f"    Avg MaxDD: {avg_maxdd:.2f}%")
        print(f"    Avg WinRate: {avg_winrate:.1f}%")
    ...
    data = {
        'strategy': 'v6.0_live_chips_weekly_wf',
        'config': {
            'oamv_data_source': oamv_filter.data_source,
            ...
            'exit_rules': [
                ...
                '8% trailing stop',
                '1.5x ATR trailing',
                'oamv_state == 0 OR weekly_state == 0',
            ],
        },
    ...
    report_file = output_dir / f"v6.0_wf_{timestamp}.json"
    with open(report_file, 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=2, default=str)

    print(f"\nResults saved: {report_file}")
    print("\n" + "=" * 120)
    print("v6.0 run complete!")
    print("=" * 120)
